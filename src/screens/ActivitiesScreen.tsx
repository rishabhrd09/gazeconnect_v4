...
const ActivitiesScreen: React.FC<{ onNavigate: (s: string) => void; onSpeak: (t: string) => void; isDarkMode?: boolean; showHindi?: boolean; }> = ({
  ...
  const pageBg = isMix ? '#17130F' : isWarm ? '#FAF5ED' : isWarmMode ? '#131412' : colors.background.primary;
  const titleText = isMix ? '#FFF0D2' : isWarm ? '#26342D' : isWarmMode ? '#ECEDE3' : colors.text.primary;
  const cardBg = isMix ? mixColors.home.tileSurfaces.ac : isWarm ? '#FCF8F2' : isWarmMode ? screenThemes.activities.cardBg : colors.background.secondary;
  const cardText = isMix ? mixColors.home.text : isWarm ? '#26342D' : isWarmMode ? '#ECEDE3' : colors.text.primary;
  const cardBorder = isMix ? `1.5px solid ${mixColors.home.cardBorder}` : isWarm ? '1.5px solid #D6CBBB' : isWarmMode ? screenThemes.activities.cardBorder : `1.5px solid ${colors.border.light}`;
  ...
  const cardShadow = isMix ? mixColors.home.cardShadow : isWarm ? 'none' : isWarmMode ? 'none' : '0 2px 8px rgba(139, 121, 104, 0.10), 0 1px 2px rgba(139, 121, 104, 0.06)';
  const backCardBg = isMix ? '#28321F' : isWarm ? '#F4EFE7' : isWarmMode ? screenThemes.activities.sidebarBg : '#DCE2C8';
