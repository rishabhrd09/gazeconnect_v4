...
const BASE_SNAP_RADIUS = 120;
/** Edge button snap radius multiplier — edge buttons get bigger snap zones */
const EDGE_SNAP_MULTIPLIER = 1.15;
// v16: Increased snap strength for ALL buttons (was 0.16 — cursor drifted past buttons).
// Stronger center-pull makes dwell targeting more accurate on Home, QuickWords, Medical, etc.
const MAX_SNAP_STRENGTH = 0.22;
...
function getSnapConfig(target: SnapTarget): { radius: number; maxStrength: number } {
  const context = (target.element?.getAttribute('data-gaze-context') || '').toLowerCase();
  if (target.priority >= 3 || context === 'gazetoggle') {
    return { radius: TOGGLE_SNAP_RADIUS, maxStrength: TOGGLE_MAX_SNAP_STRENGTH };
  }
  if (context === 'keyboard') {
    return { radius: KEYBOARD_SNAP_RADIUS, maxStrength: KEYBOARD_MAX_SNAP_STRENGTH };
  }
  if (context === 'prediction') {
    return { radius: PREDICTION_SNAP_RADIUS, maxStrength: PREDICTION_MAX_SNAP_STRENGTH };
  }
  return {
...
export function computeSnap(
  ...
  // v11: Gaze toggle buttons (priority 3) get extra pull — makes enable button easier to reach
  const { radius: snapRadius, maxStrength } = getSnapConfig(bestTarget);
  const proximity = 1 - bestDistance / snapRadius;
  const snapStrength = Math.min(maxStrength, maxStrength * proximity * proximity);
