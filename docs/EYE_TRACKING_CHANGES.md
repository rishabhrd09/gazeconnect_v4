...
## Entry 35 — 2026-07-07 — Local LLM prediction, Phase 0 (branch: feature/local_llm_keyboard_suggestion)

Kicked off the biggest planned prediction upgrade: replacing/augmenting the 661-token CIFG-LSTM with a small **local** LLM so the intended word/short phrase hits the top-5 far more often (the patient should rarely type letter-by-letter). Validated direction — Google SpeakFaster (Nature Comms 2024) saved 57% of motor actions for eye-gaze ALS users doing exactly this. Decisions locked with the patient: **local-only (no cloud), benchmark the real machine first.** Full plan in the plans folder.

**Phase 0 (this entry) — NO behavior change; new standalone files only, benchmark-gated before any integration:**
- `python/ml/llm_predictor.py` — model-agnostic, fully-guarded onnxruntime-genai wrapper (`next_words`, `complete_phrase`); hard wall-clock timeout; import-guarded so a missing runtime/model → `available=False` → returns empty → app runs identically; output filtered through `prediction_guardrails`.
- `python/ml/llm_benchmark.py` — `python -m ml.llm_benchmark` measures load time, RAM, next-word/phrase latency (p50/p95), and prints top-5 samples on ~14 realistic AAC contexts (English + Hinglish) + a verdict, to pick the largest model tier the machine safely supports.
- `docs/local-llm-benchmark-guide.md` — install/build/run instructions + the final gate (run the app, confirm `[LATENCY]` <1 ms and no new frame gaps while the model is active).

Verified: both modules `py_compile` clean; degrade to no-op without onnxruntime-genai (confirmed); existing prediction pipeline untouched (`test_prediction_pipeline.py` 11/11). **Next:** patient runs the benchmark; results decide the model tier; Phase 1 wires it into `_get_predictions` behind a default-OFF `llmPredict` flag (executor + debounce + timeout + fallback), measured via `predictionTelemetry` before flipping default.
